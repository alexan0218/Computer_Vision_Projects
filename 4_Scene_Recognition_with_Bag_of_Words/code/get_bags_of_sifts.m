...
for i = 1 : N
    % Build a histogram to record appearances
    cluster_histogram = zeros(vocab_size, 1);
    curr_img = im2single(imread(image_paths{i}));
    % Get SIFT features with step = 10
    [locations, SIFT_features] = vl_dsift(curr_img, 'Step', 10);
    SIFT_features = single(SIFT_features);
    distances = vl_alldist2(vocab,SIFT_features);
    % Kmeans with k = 10
    [sorted_dist, order] = sort(distances, 1);
    for j = 1 : vocab_size
        cluster_histogram(order(1:10,j)) = cluster_histogram(order(1:10,j)) + 1;
    end
    % Normalize histogram
    cluster_histogram = cluster_histogram / norm(cluster_histogram);
    image_feats(i, :) = cluster_histogram';
end
