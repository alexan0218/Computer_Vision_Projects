...
W = zeros(d, num_categories); % Store classifier weight vectors
B = zeros(num_categories, 1); % Store classifier offsets
LAMBDA = 0.0001;

for i = 1 : num_categories
    labels = zeros(N, 1) - ones(N, 1); % Binary labels: fill with -1
    labels(strcmp(categories{i}, train_labels)) = 1; % Current label is 1
    [w, b] = vl_svmtrain(train_image_feats', labels, LAMBDA);
    W(:, i) = w;
    B(i) = b;
end

B = repmat(B, 1, M) % Pad B for matrix-wise addition
% The category with the highest weight will be predicted
[max_val , index] = max(W' * test_image_feats' + B);
